# Storyboards — Grupo 02

---

## Histórico de Versão

| Data | Versão | Descrição | Autor(es) | Revisor(es) |
|:----:|:------:|:----------|:---------:|:-----------:|
| 19/05/2026 | 1.0 | Criação do documento de índice de storyboards | Tiago | Bryan |

---

## Tabela de Contribuição

| Integrante | Contribuição |
|:----------:|:-------------|
| Tiago | Criação do documento de índice de storyboards |
| Bryan | Adição do hyperlink para a atividade de mesario |
| Samuel Felipe | Adição do hyperlink para a atividade de quitação eleitoral |
| Lucas Fujimoto | Adição do hyperlink para a atividade de agendamento presencial |


<div style="text-align: left">
<p>Tabela 1: Tabela de contribuição (Fonte: autor, 2026).</p>
</div>

---

## Introdução

Este documento centraliza o acesso a todos os storyboards produzidos pelo Grupo 02 ao longo do projeto. Cada storyboard corresponde a uma tarefa identificada nos cenários de uso do sistema e foi desenvolvido como protótipo de baixa fidelidade, com o objetivo de representar visualmente a sequência de interações do usuário e subsidiar as etapas de design e avaliação previstas na disciplina de Interação Humano-Computador.

Os storyboards estão organizados por tarefa e podem ser acessados individualmente pelos links disponíveis na Tabela 2.

---

## Storyboards

| # | Tarefa | Responsável | Link |
|:-:|:-------|:-----------:|:----:|
| 1 | Comparação de gastos entre candidatos | Tiago | [Acessar](storyboards/ComparaçãoCand_StoryBoard.md) |
| 2 | Ser Mesario | Bryan | [Acessar](storyboards/SerMesario.md) |
| 3 | Certidão de quitação eleitoral | Samuel Felipe | [Acessar](storyboards/certidaoquitaçaoeleitor.md) |
| 4 | Agendamento Presencial | Lucas Fujimoto | [Acessar](storyboards/AgendamentoPres_StoryBoard.md) |
| 5 | Justificativa Eleitoral | Luan  Ludry| [Acessar](storyboards/JustificativaEleitoral_StoryBoard.md) |
| 6 | Alterar local de voto | Guilherme | [Acessar](storyboards/alterarlocal.md) |
| 7 | Emissão 1° Titulo de Eleitor | Maria Luana | [Acessar](storyboards/Emissão1°Titulo.md) |

<div style="text-align: left">
<p>Tabela 2: Índice de storyboards do Grupo 02 (Fonte: autor, 2026).</p>
</div>

---


## Referência Bibliográfica

> <a id="REF1" href="#anchor_1">1.</a> PREECE, Jennifer; SHARP, Helen; ROGERS, Yvonne. **Interaction Design: beyond human-computer interaction**. John Wiley & Sons, 2015.

> <a id="REF2" href="#anchor_2">2.</a> KLEMMER, Scott. **Storyboards, Paper Prototypes and Mockups**. Univ. Califórnia em Berkeley (Coursera). Disponível em: [https://www.youtube.com/watch?v=h2H3oIQtddU](https://www.youtube.com/watch?v=h2H3oIQtddU). Acesso em: 19 mai. 2026.

---

## Agradecimentos

Agradecemos à IA Generativa **Claude** (Anthropic) pelo suporte na elaboração deste documento. A ferramenta foi utilizada para auxiliar na estruturação do índice, na redação da introdução e na formatação das tabelas e hiperlinks. Todo o conteúdo técnico — incluindo a definição das tarefas, o desenvolvimento dos storyboards e as decisões de design — foram realizados pelos integrantes da equipe; o Claude atuou como assistente de formatação e redação, sem interferir nas escolhas metodológicas do grupo.
